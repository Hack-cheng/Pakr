...
class MainActivity : AppCompatActivity() {
    ...
    private fun inspectBlankPage(view: WebView, url: String) {
        ...
        view.postVisualStateCallback(System.currentTimeMillis(), object : WebView.VisualStateCallback() {
            ...
            view.evaluateJavascript(
                ...
                "return JSON.stringify({textLength:text.length,htmlLength:html.length,title:document.title||'',bg:bg});" +
    ...
    private fun errorHtml(url: String?, errDesc: String? = null): String {
        ...
        return "<!DOCTYPE html>" +
            ...
            "font-family:-apple-system,sans-serif;flex-direction:column;background:#fff;color:#1a1a1a;" +
            "padding:32px;box-sizing:border-box;text-align:center;\">" +
            "<svg width=\"56\" height=\"56\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"#d1d5db\" " +
            "stroke-width=\"1.5\" style=\"margin-bottom:8px;flex-shrink:0\">" +
            "<circle cx=\"12\" cy=\"12\" r=\"9\"/><path d=\"M4.93 4.93l14.14 14.14\"/></svg>" +
            "<p style=\"margin:12px 0 6px;font-size:17px;font-weight:600;color:#111;\">网页加载失败</p>" +
            "<p style=\"margin:0 0 28px;font-size:13px;color:#888;max-width:260px;line-height:1.6\">" +
            safeDesc + "</p>" +
            "<button onclick=\"if(window.NativeBridge){NativeBridge.reload()}" +
            "else{window.location.href='" + safeUrl + "'}\" " +
            "style=\"padding:13px 36px;border:none;border-radius:999px;background:#111;color:#fff;" +
            "font-size:15px;cursor:pointer;font-family:-apple-system,sans-serif;font-weight:500;" +
            "-webkit-tap-highlight-color:transparent\">重试</button>" +
            "</body></html>"
    }
